...
coordinate select(){
    ...
    cout <<  endl << "Choose the cell you want! " << endl << "Input the row: (row = "  << player << ") ";
    cin >> wantedPosition.row;

    cout << "Input the column: (1 <= column <= 5) " ;
    cin >> wantedPosition.column;

    cout << "Do you want to go the the left or the right? "  << endl << "right = counterclockwise, left = clockwise " ;
